...
export function ModalCreateMovie({
  isOpen,
  onClose,
  onComplete,
}: ModalCreateMovieProps) {
  ...
  async function onSubmit(data: CreateMovieFormValues) {
    ...
    if (result.type === "movies/createMovie/fulfilled") {
      onComplete();
      onClose();
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Tạo phim mới</DialogTitle>
        </DialogHeader>
