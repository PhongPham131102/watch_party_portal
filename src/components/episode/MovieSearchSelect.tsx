...
export function MovieSearchSelect({
  value,
  onChange,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          type="button"
          className={`w-[220px] justify-between ${className || ""}`}>
          <span className="truncate text-sm">
            {selectedMovie ? selectedMovie.title : "Tất cả phim"}
          </span>
      ...
      <PopoverContent className="w-[320px] p-0">
        ...
        <div className="p-2 border-b border-gray-100 flex items-center justify-between">
          <span className="text-xs text-gray-500">
            {selectedMovie
              ? "Đang lọc theo phim đã chọn"
              : "Không chọn = tất cả phim"}
          </span>
          {selectedMovie && (
            <Button
              type="button"
            ...
            <div className="divide-y divide-gray-100">
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-50 ${
                  !selectedMovie ? "bg-blue-50 text-blue-600" : ""
                }`}
                onClick={() => handleSelect(null)}>
                Tất cả phim
              </button>
